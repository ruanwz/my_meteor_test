// Request Zyncro credentials for the user
// @param options {optional}  XXX support options.requestPermissions
// @param credentialRequestCompleteCallback {Function} Callback function to call on
//   completion. Takes one argument, credentialToken on success, or Error on
//   error.
Zyncro.requestCredential = function (options, credentialRequestCompleteCallback) {
  ...
  var config = ServiceConfiguration.configurations.findOne({service: 'zyncro'});
  if (!config) {
  ...
  var callbackUrl = Meteor.absoluteUrl('_oauth/zyncro?close&state=' + credentialToken);

  // url to app, enters "step 1" as described in
  // packages/accounts-oauth1-helper/oauth1_server.js
  var url = '/_oauth/zyncro/?requestTokenAndRedirect='
